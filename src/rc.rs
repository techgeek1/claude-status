...
/// Subset of `~/.claude/sessions/<pid>.json`. Claude Code merges patches into
/// this file as session state changes; `bridgeSessionId` is set when a remote
/// control client attaches and written back as `null` when it detaches.
#[derive(Deserialize)]
struct SessionFile {
    pid: i32,
    cwd: String,
